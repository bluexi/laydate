...
//切换可用性
    Dates.enabled = function (elem, status) {
        if (status) {
            Dates.removeCssAttr(elem, 'disabled');
            Dates.removeCssAttr(elem, 'backgroundColor');
        } else {
            elem.disabled = true;
...
/*     Dates.timeVoid = function (times, index) {
	...
	//修复如果最大值最小是显示的为同一个值的时候，只能有一个有效的问题
	//时分秒的有效检测
    Dates.timeVoid = function (times, index) {
        if (Dates.ymd[1] + 1 == Dates.mins[1] && Dates.ymd[2] == Dates.mins[2]) {
            if (index === 0 && (times < Dates.mins[3])) {
		...
		if (Dates.ymd[1] + 1 == Dates.maxs[1] && Dates.ymd[2] == Dates.maxs[2]) {
            if (index === 0 && times > Dates.maxs[3]) {
                return 1;
            } else if (index === 1 && times > Dates.maxs[4]) {
                return 1;
...
//是否显示零件
    ...
    Dates.timeable = function () {
        var S = Dates.query, log = {
            h: S('#laydate_hms input')[0],
            m: S('#laydate_hms input')[1],
            s: S('#laydate_hms input')[2]
        };
        var timerange = Dates.options.timerange || 'hms';
        if (timerange.indexOf('h') >= 0) {
            Dates.enabled(log.h, true);
        } else {
            Dates.enabled(log.h, false);
        }
        if (timerange.indexOf('m') >= 0) {
            Dates.enabled(log.m, true);
        } else {
            Dates.enabled(log.m, false);
        }
        if (timerange.indexOf('s') >= 0) {
            Dates.enabled(log.s, true);
        } else {
            Dates.enabled(log.s, false);
        }
    }
